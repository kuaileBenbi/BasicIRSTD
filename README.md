...
* **April 3, 2026: Add pluggable neck comparison models.**

  **New folder `model/NeckCompare/`: Add a shared lightweight residual backbone with four interchangeable neck modules: `SPP`, `FPN`, `PANet`, and `ACM`.** `<br>`
  **New model names: `CompareSPP`, `CompareFPN`, `ComparePANet`, `CompareACM`. These names are registered in `model/__init__.py` and `net.py` for direct use in training, test, inference, evaluation, and parameter/FLOPs calculation.**
  `<br><br>`
...
* **Run **`train.py`** to perform network training in single GPU and multiple GPUs. Example for training [model_name] on [dataset_name] datasets:**
  ...
  $ python train.py --model_names CompareSPP CompareFPN ComparePANet CompareACM --dataset_names SIRST-v1
  ```
...
### Built-in pluggable neck comparison models

* **A ready-to-use ablation model family is provided in** **`./model/NeckCompare/`** **for fast comparison of multi-scale fusion modules.**
* **Shared backbone:** `ModularIRSTDNet` uses one lightweight residual encoder and one segmentation head, so the main variable is the neck design.
* **Supported necks:** `SPP`, `FPN`, `PANet`, `ACM`.
* **Registered model names:** `CompareSPP`, `CompareFPN`, `ComparePANet`, `CompareACM`.
* **Implementation files:**
  ```
  ./model/NeckCompare/
  ├── __init__.py
  └── model_NeckCompare.py
  ```
* **Main implementation entry:**
  ```python
  from model.NeckCompare import ModularIRSTDNet

  model = ModularIRSTDNet(neck_type='panet')
  ```
* **Wrapper classes for direct use with the existing scripts:**
  ```python
  from model.NeckCompare.model_NeckCompare import (
      CompareSPP,
      CompareFPN,
      ComparePANet,
      CompareACM,
  ...
  ```
* **Design note:** `CompareACM` reuses the existing `AsymBiChaFuse` implementation from `model/ACM/fusion.py`, while the other three variants use the same encoder/head and only replace the neck.
...
* **Run **`test.py`** to perform network inference and evaluation. Example for test [model_name] on [dataset_name] datasets:**
  ...
  $ python test.py --model_names CompareSPP CompareFPN ComparePANet CompareACM --dataset_names SIRST-v1
  ```
...
* **Run **`inference.py`** to inference only with images. Examples:**
  ...
  $ python inference.py --model_names CompareACM --dataset_names SIRST-v1
  ```
* **Network predictions will be saved to** **`./results/`** **that has the following structure**:
  ```
  ├──./results/
...
* **Run **`evaluate.py`** for direct eevaluation. Example for evaluate [model_name] on [dataset_name] datasets:**
  ...
  $ python evaluate.py --model_names CompareSPP CompareFPN ComparePANet CompareACM --dataset_names SIRST-v1
  ```
...
* **Run **`cal_params.py`** for parameters and FLOPs calculation. Examples:**
  ...
  $ python cal_params.py --model_names CompareSPP CompareFPN ComparePANet CompareACM
  ```
