...
parser.add_argument(
    "--dataset_dir", default="./datasets", type=str, help="train_dataset_dir"
)
parser.add_argument("--batchSize", type=int, default=16, help="Training batch sizse")
parser.add_argument("--patchSize", type=int, default=256, help="Training patch size")
parser.add_argument(
    "--save", default="./log", type=str, help="Save path of checkpoints"
)
parser.add_argument(
    "--resume",
    default=None,
    nargs="+",
    help="Resume from exisiting checkpoints (default: None)",
)
parser.add_argument(
    "--pretrained",
    default=None,
    nargs="+",
    help="Load pretrained checkpoints (default: None)",
)
...
parser.add_argument(
    ...
    default="Adam",
    type=str,
    help="optimizer name: Adam, Adagrad, SGD",
)
parser.add_argument(
    "--optimizer_settings", default={"lr": 5e-4}, type=dict, help="optimizer settings"
)
parser.add_argument(
    "--scheduler_name",
    default="MultiStepLR",
    type=str,
    help="scheduler name: MultiStepLR",
)
parser.add_argument(
    "--scheduler_settings",
    default={"step": [200, 300], "gamma": 0.5},
    type=dict,
...
parser.add_argument(
    "--intervals", type=int, default=10, help="Intervals for print loss"
)
...
parser.add_argument(
    ...
    type=str,
    default="auto",
    help="Training device: 'auto', 'cpu', or a CUDA device such as 'cuda:0'",
)
...
def train():
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        for idx_iter, (img, gt_mask) in enumerate(train_loader):
            ...
            gt_mask = Variable(gt_mask).to(opt.device)
            if img.shape[0] == 1:
                continue
        ...
        if (idx_epoch + 1) % opt.intervals == 0:
            ...
            print(
                time.ctime()[4:-5]
                + " Epoch---%d, total_loss---%f," % (idx_epoch + 1, total_loss_list[-1])
            )
            opt.f.write(
                time.ctime()[4:-5]
                + " Epoch---%d, total_loss---%f,\n"
...
def test(save_pth):
    ...
    ckpt = torch.load(save_pth, map_location=opt.device)
    net.load_state_dict(ckpt["state_dict"])
    net.eval()
    ...
    with torch.no_grad():
        for idx_iter, (img, gt_mask, size, _) in enumerate(test_loader):
            ...
            eval_PD_FA.update(
                (pred[0, 0, :, :] > opt.threshold).cpu(), gt_mask[0, 0, :, :], size
            )
