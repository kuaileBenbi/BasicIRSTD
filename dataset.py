...
class TrainSetLoader(Dataset):
    def __init__(self, dataset_dir, dataset_name, patch_size, img_norm_cfg=None):
        ...
        if not os.path.exists(
            self.dataset_dir + "/img_idx/train_" + dataset_name + ".txt"
        ) and os.path.exists(self.dataset_dir + "/img_idx/train.txt"):
            shutil.copyfile(
                self.dataset_dir + "/img_idx/train.txt",
                self.dataset_dir + "/img_idx/train_" + dataset_name + ".txt",
            )
        with open(
            self.dataset_dir + "/img_idx/train_" + dataset_name + ".txt", "r"
        ) as f:
    ...
    def __getitem__(self, idx):
        try:
            img = Image.open(
                (self.dataset_dir + "/images/" + self.train_list[idx] + ".png").replace(
                    "//", "/"
                )
            ).convert("I")
            mask = Image.open(
                (self.dataset_dir + "/masks/" + self.train_list[idx] + ".png").replace(
                    "//", "/"
                )
            )
        except:
            img = Image.open(
                (self.dataset_dir + "/images/" + self.train_list[idx] + ".bmp").replace(
                    "//", "/"
                )
            ).convert("I")
            mask = Image.open(
                (self.dataset_dir + "/masks/" + self.train_list[idx] + ".bmp").replace(
                    "//", "/"
                )
            )
...
class TestSetLoader(Dataset):
    def __init__(
        self, dataset_dir, train_dataset_name, test_dataset_name, img_norm_cfg=None
    ):
        super(TestSetLoader).__init__()
        self.dataset_dir = dataset_dir + "/" + test_dataset_name
        with open(
            self.dataset_dir + "/img_idx/test_" + test_dataset_name + ".txt", "r"
        ) as f:
    ...
    def __getitem__(self, idx):
        try:
            img = Image.open(
                (self.dataset_dir + "/images/" + self.test_list[idx] + ".png").replace(
                    "//", "/"
                )
            ).convert("I")
            mask = Image.open(
                (self.dataset_dir + "/masks/" + self.test_list[idx] + ".png").replace(
                    "//", "/"
                )
            )
        except:
            img = Image.open(
                (self.dataset_dir + "/images/" + self.test_list[idx] + ".bmp").replace(
                    "//", "/"
                )
            ).convert("I")
            mask = Image.open(
                (self.dataset_dir + "/masks/" + self.test_list[idx] + ".bmp").replace(
                    "//", "/"
                )
            )
...
class EvalSetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_gt = Image.open(
            self.dataset_dir + "/masks/" + self.test_list[idx] + ".png"
        )
