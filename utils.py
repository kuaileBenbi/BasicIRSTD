...
def random_crop(img, mask, patch_size, pos_prob=None):
    ...
    if pos_prob == None or cur_prob > pos_prob or mask.max() == 0:
        ...
        h_start = random.randint(
            max(0, loc[0][idx] - patch_size), min(loc[0][idx], h - patch_size)
        )
        w_start = random.randint(
            max(0, loc[1][idx] - patch_size), min(loc[1][idx], w - patch_size)
        )
...
def get_img_norm_cfg(dataset_name, dataset_dir):
    if dataset_name == "NUAA-SIRST":
        ...
        if os.path.exists(
            dataset_dir + "/" + dataset_name + "/img_idx/test_" + dataset_name + ".txt"
        ):
            with open(
                dataset_dir
                + "/"
        ...
        for img_pth in img_list:
            try:
                img = Image.open(
                    (img_dir + img_pth).replace("//", "/") + ".png"
                ).convert("I")
            except:
                try:
                    img = Image.open(
                        (img_dir + img_pth).replace("//", "/") + ".jpg"
                    ).convert("I")
                except:
                    img = Image.open(
                        (img_dir + img_pth).replace("//", "/") + ".bmp"
                    ).convert("I")
            img = np.array(img, dtype=np.float32)
            mean_list.append(img.mean())
            std_list.append(img.std())
        img_norm_cfg = dict(
            mean=float(np.array(mean_list).mean()), std=float(np.array(std_list).mean())
        )
...
def get_optimizer(
    net, optimizer_name, scheduler_name, optimizer_settings, scheduler_settings
):
